...
public class Process {
	...
	public void proceed() {
		...
		//while (true) {
			...
			int maximumProductList = allProductSetWithMinSup.get(allProductSetWithMinSup.size() - 1).productList.size();

			for (int i = allProductSetWithMinSup.size() - 1; i > 0; i--) {
				if (allProductSetWithMinSup.get(i).productList.size() == 1)
					break;
				printConfidence(allProductSetWithMinSup.get(i));
			}

			System.out.print("Please enter new products: ");
			String products = sc.nextLine();
			new DatabaseTest().insertIntoApTable(products);
	...
	private void printConfidence(ProductSet testList) {
		// cout << endl;

		System.out.println();
		for (int i = 0; i < allProductSetWithMinSup.size(); i++) {
			if (allProductSetWithMinSup.get(i).productList.size() < testList.productList.size()) {

				if (subsetOf(allProductSetWithMinSup.get(i), testList)) {

					for (int k = 0; k < allProductSetWithMinSup.get(i).productList.size(); k++) {

						System.out.print(allProductSetWithMinSup.get(i).productList.get(k) + " ");
					...
					for (int k = 0; k < testList.productList.size(); k++) {
						System.out.print(testList.productList.get(k) + " ");
						// cout << testList.productList[k] << " ";
					}

					System.out.println("       confidence  =  " + testList.count + "/"
							+ allProductSetWithMinSup.get(i).count + "  =  "
							+ ((double) testList.count / allProductSetWithMinSup.get(i).count) * 100 + "%");
					// cout << " confidence = " << testList.count << "/" <<
					// allProductSetWithMinSup[i].count << " = " <<
					// ((double)testList.count/allProductSetWithMinSup[i].count)*100
					// << "%" << endl;
					System.out.println();
					// cout << endl;
	...
	private boolean subsetOf(ProductSet subset, ProductSet testList) {
		int count = 0;
		for (int i = 0; i < subset.productList.size(); i++)
			for (int j = 0; j < testList.productList.size(); j++)
				if (subset.productList.get(i).equals(testList.productList.get(j))) {
					count++;
					break;
				}
		if (count == subset.productList.size())
			return true;
		return false;
	}
	...
	private void operate(ArrayList<ProductSet> setofProductList, ArrayList<ArrayList<String>> transactions) {
		...
		System.out.println();
		if (selectedSetOfProductList.size() < 1)
			return;
		...
		if (selectedSetOfProductList.get(0).productList.size() == 1) {
			...
			for (int i = 0; i < selectedSetOfProductList.size() - 1; i++)
				for (int j = i + 1; j < selectedSetOfProductList.size(); j++)
					if (selectedSetOfProductList.get(i).productList.get(0)
							.equals(selectedSetOfProductList.get(j).productList.get(0))) {

						ProductSet tempCombination = getNewCombination(selectedSetOfProductList.get(i),
								selectedSetOfProductList.get(j));
						if (suffexExist(selectedSetOfProductList, tempCombination))
							newSetofProductList.add(tempCombination);
					}
	...
	private boolean suffexExist(ArrayList<ProductSet> selectedSetOfProductList, ProductSet tempCombination) {
		for (int i = 0; i < selectedSetOfProductList.size(); i++) {
			int count = 0;
			for (int j = 0; j < selectedSetOfProductList.get(i).productList.size(); j++) {
				for (int k = 1; k < tempCombination.productList.size(); k++) {
					if (tempCombination.productList.get(k).equals(selectedSetOfProductList.get(i).productList.get(j)))
						count++;
				}
			}

			if (count == selectedSetOfProductList.get(i).productList.size()) {
				// cout << count << endl;
				return true;
	...
	private void selectProductList(ArrayList<ProductSet> selectedSetOfProductList,
			ArrayList<ProductSet> setofProductList, ArrayList<ArrayList<String>> transactions) {
	...
	private void inputTransactions() {
		...
		/*
		 *
		 * for(int i = 0; i < transactions.size(); i++) { for(int j = 0; j <
		 * transactions.get(i).size(); j++) {
